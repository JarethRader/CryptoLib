(function(exports) {
  ...
  exports.ReaderControl = function(options) {
    ...
    this.useDownloader = _.isUndefined(options.useDownloader)
      ? true
      : !!options.useDownloader;
    this.forceClientSideInit = options.forceClientSideInit;
    ...
    var autoLoadOffice =
      this.officeType === "auto" || this.officeType === "wait";
    ...
    this.pdfTypePromise = autoLoadPdf
      ? defaultBackendTypePromise
      : Promise.resolve(this.pdfType);
    this.officeTypePromise = autoLoadOffice
      ? defaultBackendTypePromise
      : Promise.resolve(this.officeType);

    if (options.preloadWorker) {
      if (this.pdfType !== "wait") {
        this.pdfTypePromise.then(function(pdfType) {
          var useEmscriptenWhileLoading =
            me.pdfType !== "pnacl" && !exports.CoreControls.isSubzeroEnabled();
      ...
      if (this.officeType !== "wait") {
        this.officeTypePromise.then(function(officeType) {
          var useEmscriptenWhileLoading =
            me.officeType !== "pnacl" &&
            !exports.CoreControls.isSubzeroEnabled();
          exports.CoreControls.preloadOfficeWorker(
            officeType,
            me.workerHandlers,
            {
              useEmscriptenWhileLoading: useEmscriptenWhileLoading,
    ...
    me.handlePassword = function(options) {
      ...
      if (me.passwordTries === 0) {
        ...
        var close = function() {
          if (!tryingPassword) {
            me.fireError(
              "The document requires a valid password.",
              i18n.t("error.EncryptedUserCancelled")
            );
          }
        };
        ...
        me.fireError(
          "The document requires a valid password.",
          i18n.t("error.EncryptedAttemptsExceeded")
        );
      } else {
    ...
    me.onDocError = function(err) {
      ...
      me.fireError(
        err.message,
        i18n.t(
          me.usingOfficeWorker ? "error.OfficeLoadError" : "error.PDFLoadError"
        )
      );
    };
  };
  ...
  exports.ReaderControl.prototype = {
    ...
    initUI: function() {
      ...
      if (this.showFilePicker) {
        ...
        var filterString =
          "." +
          this.supportedPDFExtensions.join(",.") +
          (hasPDFExtensions && hasOfficeExtensions ? ",." : "") +
          this.supportedOfficeExtensions.join(",.");
        var $filePicker = $(
          '<span class="mobile-button"><label for="input-pdf" class="file-upload glyphicons folder_open"></label></span>' +
            '<input id="input-pdf"  accept="' +
            filterString +
            '" type="file" class="input-pdf">'
        )
        ...
        uiDisplayElement.addEventListener("drop", function(e) {
          ...
          if (
            ...
            (firstFile.type === "application/pdf" ||
              ...
              (firstFile.name.length > 3 &&
                firstFile.name.lastIndexOf(".md") ===
                  firstFile.name.length - 3))
          ) {
    ...
    createPDFDownloadButton: function() {
      ...
      var downloadButton = $("<span></span>")
        .attr({ id: "PDFDownloadButtonContainer" })
    ...
    createOfficeDownloadButton: function() {
      ...
      var me = this;
      var downloadButtonPDF = $("<div></div>")
        .addClass("labelled-button-row")
        .append(
          $("<span></span>")
        ...
        .append(
          $("<span></span>")
            .addClass("heading labelled-button-text")
            .attr({
              "data-i18n":
                "[title]controlbar.convertToPDF;controlbar.convertToPDF"
            })
            .i18n()
        );
      var downloadOfficeButton = $("<div></div>")
        .addClass("labelled-button-row")
        .append(
          $("<span></span>")
        ...
        .append(
          $("<span></span>")
            .addClass("heading labelled-button-text")
            .attr({
              "data-i18n": "[title]controlbar.download;controlbar.download"
            })
            .i18n()
        );

      var outer = $("<span></span>")
        .attr({ id: "OfficeDownloadButtonContainer" })
        .addClass("mobile-button");

      $("<span></span>")
        .addClass("current-layout drop-target glyphicons disk_save")
        .appendTo(outer);
      var current = $("<div></div>")
        .addClass("hidden")
        .appendTo(outer);
      current = $("<div></div>")
        .addClass("content  download-drop")
        .appendTo(current);
      ...
      var drop = new Drop({
        target: document.querySelector(
          "#OfficeDownloadButtonContainer .drop-target"
        ),
        content: document.querySelector(
          "#OfficeDownloadButtonContainer .content"
        ),
    ...
    downloadFile: function(options) {
      ...
      var freeHandCompletePromise = this.toolModeMap[
        exports.PDFTron.WebViewer.ToolMode.AnnotationCreateFreeHand
      ].complete();
      ...
      Promise.all([
        me.docViewer.getAnnotationsLoadedPromise(),
        freeHandCompletePromise
      ]).then(function() {
        var annotManager = me.docViewer.getAnnotationManager();
        options["xfdfString"] =
          options["xfdfString"] || annotManager.exportAnnotations();

        if (!("flags" in options)) {
          var signatures = annotManager
            .getAnnotationsList()
        ...
        if (!me.saveCancelled) {
          ...
          if (bbURLPromise) {
            ...
            downloadButton.append(
              '<iframe width="0" height="0" id="dlFrame"></iframe>'
            );
    ...
    getDownloadFilename: function(filename, extension) {
      if (
        filename &&
        filename.slice(-extension.length).toLowerCase() !== extension
      ) {
        filename += extension;
      }
      return filename;
    ...
    loadDocumentConfirm: function() {
      ...
      return new Promise(function(resolve, reject) {
        ...
        var loadMessage = $("<label>")
          .text(
            "Are you sure you want to cancel the current document download and load the new document?"
          )
      ...
      this.loadDocumentConfirm().then(
        function() {
          ...
          if (me.blackBoxServer) {
            partRetriever = new CoreControls.PartRetrievers.BlackBoxPartRetriever(
              doc,
              me.blackBoxServer,
              {
                ...
                cacheKey: options && options.cacheKey
              }
            );
          } else {
            partRetriever = new CoreControls.PartRetrievers.ExternalPdfPartRetriever(
              doc,
              {
          ...
          partRetriever.on("documentLoadingProgress", function(
            e,
            loaded,
            total
          ) {
    ...
    loadLocalFile: function(file, options) {
      ...
      this.loadDocumentConfirm().then(
        function() {
          ...
          var partRetriever = new CoreControls.PartRetrievers.LocalPdfPartRetriever(
            file
          );
          partRetriever.on("documentLoadingProgress", function(
            e,
    ...
    loadAsync: function(id, partRetriever, options) {
      ...
      var defaultOptions = {
        ...
        forceClientSideInit: me.forceClientSideInit
      };
        ...
        if (filename) {
          ...
          for (var i = 0; i < supportedExtensions.length; ++i) {
            if (lowered.endsWith("." + supportedExtensions[i])) {
              extension = lowered.substr(
                lowered.length - supportedExtensions[i].length
              );
              break;
            }
          }
      ...
      if (me.blackBoxServer) {
        extension =
          getExtension(me.filename, this.supportedBlackboxExtensions) ||
          extension;
        ...
        me.bbAnnotManager = new CoreControls.BlackBoxAnnotationManager(
          me.blackBoxServer,
          me.docViewer
        );
        return;
      ...
      extension = getLowered(
        getExtension(me.filename, this.supportedPDFExtensions) || extension
      );
      var officeExtension = getLowered(
        getExtension(me.filename, this.supportedOfficeExtensions)
      );
      if (officeExtension) {
      ...
      (useOfficeWorker ? this.officeTypePromise : this.pdfTypePromise).then(
        function(backendType) {
          ...
          if (useOfficeWorker && !options.officeWorkerTransportPromise) {
            options.workerTransportPromise = CoreControls.initOfficeWorkerTransports(
              backendType,
              me.workerHandlers
            );
          } else if (!useOfficeWorker && !options.workerTransportPromise) {
            options.workerTransportPromise = CoreControls.initPDFWorkerTransports(
              backendType,
              me.workerHandlers
            );
          } else if (useOfficeWorker) {
            options.workerTransportPromise =
              options.officeWorkerTransportPromise;
          }
          ...
          options.workerTransportPromise.catch(function(workerError) {
            if (typeof workerError === "string") {
              ...
              me.fireError(
                workerError.message,
                i18n.t(workerError.userMessage || workerError.message)
              );
            }
          });
    ...
    initProgress: function() {
      var me = this;
      this.$progressBar = $(
        '<div id="pdf-progress-bar"><div class="progress-text"></div><div class="progress-bar"><div style="width:0%">&nbsp;</div><span>&nbsp;</span></div></div>'
      );
      ...
      $(document)
        .on("workerLoadingProgress", function(e, progress) {
          ...
          if (proPer > 0 && !failed && !finished) {
            me.$progressBar
              .find(".progress-text")
              .text(i18n.t("initialize.pnacl") + proPer + "%");
          }
          me.$progressBar
          ...
          if (progress >= 1 && !failed) {
            ...
            me.$progressBar
              .find(".progress-text")
              .text(i18n.t("Initializing") + " " + proPer + "%");
          }
        })
        .on("documentLoadingProgress", function(e, bytesLoaded, bytesTotal) {
          ...
          if (
            viewerLoadedDeferred.state() !== "pending" ||
            !me.$progressBar.hasClass("document-failed")
          ) {
            ...
            if (loadedPercent >= 0) {
              if (!me.$progressBar.hasClass("document-failed")) {
                me.$progressBar
                  .find(".progress-text")
                  .text(
                    i18n.t("initialize.loadDocument") + loadedPercent + "%"
                  );
              }
              me.$progressBar
                .find(".progress-bar div")
                .css({ width: loadedPercent + "%" });
            } else {
              var kbLoaded = Math.round(bytesLoaded / 1024);
              if (!me.$progressBar.hasClass("document-failed")) {
                me.$progressBar
                  .find(".progress-text")
                  .text(i18n.t("initialize.loadDocument") + kbLoaded + "KB");
                me.$progressBar
                  .find(".progress-bar div")
                  .css({ width: "100%" });
              }
          ...
          me.docViewer.getWatermark().then(function(watermark) {
            if (isSupported && !watermark) {
              ...
              if (!me.printCancelled) {
                currentDocument.getFileData(options).then(
                  function(data) {
                    endLoading();
                    if (!me.printCancelled) {
                  ...
                  function onDownloadFailed() {
                    ...
                    if (!me.printCancelled) {
                      ...
                      exports.DesktopReaderControl.prototype.printHandler.call(
                        me
                      );
                    }
                  }
  ...
  exports.ReaderControl.prototype = $.extend(
    {},
    exports.DesktopReaderControl.prototype,
    exports.ReaderControl.prototype
  );
})(window);
...
$("head").append(
  $('<link rel="stylesheet" type="text/css" />').attr(
    "href",
    window.assetPath + "pdf/PDFReaderControl.css"
  )
);
